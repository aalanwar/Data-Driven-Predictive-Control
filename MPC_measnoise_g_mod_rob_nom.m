...
% Paper equation (19)
% 2 method don't agree with 
% model-based, nominal, not robust 
...
uref = 8;    
ref = inv(eye(5)-sys_d.A)*sys_d.B*uref; %  [-1.4118, 2.3529, 3.2000, 1.6000, 4.0000]
...
%    Cost = Cost + (y{i+1}-ref)'*Qy*(y{i+1}-ref);
    ...
    norm(YPred_model(:,timesteps)-ref,2);
     ...
     u_model_nom = sdpvar(1*ones(1,N),ones(1,N));
    y_model_nom = sdpvar(5*ones(1,N+1),ones(1,N+1));
        alpha_u = sdpvar*ones(1,N); 
      Constraints = [y_t_model_nom(:,timesteps) == y_model_nom{1}];
     for i = 1:N
        Constraints = [Constraints,...
            y{i+1} <= y_ub,...
            y{i+1} >= y_lb,...
            u_model_nom{i} == U.center + alpha_u(i) * U.generators,...
            y_model_nom{i+1} == sys_d.A *y_model_nom{i} + sys_d.B*u_model_nom{i},...
            alpha_u(i) <= 1 , ...
            alpha_u(i) >= -1       
            ];
    end
...
%    Cost = Cost + (y{i+1}-ref)'*Qy*(y{i+1}-ref);
    ...
    %double(alpha_u(1));
    YPred_model_nom(:,timesteps+1) = double(y_model_nom{2});
    %y11(:,timesteps) = double(y{1});
    norm(YPred_model_nom(:,timesteps)-ref,2);
...
%     uPred_final(timesteps) = uPred_;  
    ...
    y_t_model_nom(:,timesteps+1) = sys_d.A * y_t_model_nom(:,timesteps) + sys_d.B * uPred_model_nom(timesteps) + w_point +v_point - sys_d.A *v_point;

    yt2ref(timesteps)= norm(y_t(:,timesteps)-ref,2)
    yt2ref_model(timesteps)= norm(y_t_model(:,timesteps)-ref,2)
    yt2ref_model_nom(timesteps)= norm(y_t_model_nom(:,timesteps)-ref,2)
    halt = 1;
end
    ...
    Cost_model_nom=0;
    for i=1:timesteps
        Cost_model_nom = Cost_model_nom + (y_t_model_nom(:,i+1)-ref)'*Qy*(y_t_model_nom(:,i+1)-ref)+ (uPred_model_nom(:,i)-uref)'*Qu*(uPred_model_nom(:,i)-uref);
    end
    Cost_model=0;
    for i=1:timesteps
        Cost_model = Cost_model + (y_t_model(:,i+1)-ref)'*Qy*(y_t_model(:,i+1)-ref)+ (uPred_model(:,i)-uref)'*Qu*(uPred_model(:,i)-uref);
    end
    Cost=0;
    for i=1:timesteps
        Cost = Cost + (y_t(:,i+1)-ref)'*Qy*(y_t(:,i+1)-ref)+ (uPred(:,i)-uref)'*Qu*(uPred(:,i)-uref);
    end
...
han_pred_model_nom=plot(uPred_model_nom,'ko-');

han_uref=plot(uref*ones(size(uPred)),'k-');
ax = gca;
xlabel('Time step $k$','Interpreter','latex')
legend([han_pred,han_pred_model,han_uref,han_pred_model_nom],'ZonoPC Control input $u^*(k)$','MPC Control input $u^*(k)$','MPC nominal Control input $u^*(k)$','Reference input $r_u(k)$','Interpreter','latex')
ax.FontSize = 16;
%set(gcf, 'Position',  [50, 50, 800, 400])
ax = gca;
outerpos = ax.OuterPosition;
...
han_yt2ref_model_nom=plot(yt2ref_model_nom,'ko-');
xlabel('Time step $k$','Interpreter','latex')
legend([han_yt2ref,han_yt2ref_model,han_yt2ref_model_nom,],'ZonoPC $|| y(k) - r_y(k) ||$','MPC $|| y(k) - r_y(k) ||$','MPC nominal $|| y(k) - r_y(k) ||$','Interpreter','latex')
    ax = gca;
    ax.FontSize = 12;
    %set(gcf, 'Position',  [50, 50, 800, 400])
    ax = gca;
