function handle = plotFilledTemplate(obj,varargin)
% plotFilledTemplate - Plot an over-approximation of the 2D-projection of a 
%                      constrained zonotope with a template polyhedron
%
% Syntax:  
%    handle = plotFilledTemplate(obj)
%    handle = plotFilledTemplate(obj,dim,numDir,plotOptions)
%
% Inputs:
...
%   
%    figure
%    plotFilledTemplate(cZono,[1,2],16,'r','EdgeColor','none');
%    xlim([-2,0]);
%    ylim([-2,3]);
%
%    figure
%    plotFilledTemplate(cZono,[1,2],100,[0 0.5 0],'EdgeColor','none');
%    xlim([-2,0]);
%    ylim([-2,3]);
%
%    figure
...
%
% See also: plot, plotFilled, plotFilledSplit
...
plotFilled(poly,[1,2],plotOptions{:});
handle = gca;
