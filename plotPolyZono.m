...
legend([han_pred,han_pred_model,han_pred_poly,han_uref],'ZPC $u^*(k)$','MPC using zonotope $u^*(k)$','MPC using polytopes $u^*(k)$','Reference input $r_u(k)$','Interpreter','latex')
ax.FontSize = 16;
%set(gcf, 'Position',  [50, 50, 800, 400])
ax = gca;
outerpos = ax.OuterPosition;
...
xlabel('Time step $k$','Interpreter','latex')
legend([han_yt2ref,han_yt2ref_model,han_yt2ref_poly],'ZPC $|| y(k) - r_y(k) ||$','MPC using zonotopes $|| y(k) - r_y(k) ||$','MPC using polytopes $|| y(k) - r_y(k) ||$','Interpreter','latex')
ax = gca;
ax.FontSize = 12;
%set(gcf, 'Position',  [50, 50, 800, 400])
ax = gca;
